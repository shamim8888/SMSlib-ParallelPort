...
require("config.php");
 if ($submitted == 1)
                        ...
                        if ($filling=="viewcheck")
                                {
                                        ...
                                        $subresult = pg_exec("select * from materialone order by matone_name");
                                        ...
                                        $subresult = pg_exec("select * from materialone order by matone_id");
                                        $subnumrows = pg_numrows($subresult);
        ...
        else
                {
                        ...
                        $query_string = "select * from materialone order by matone_name";
...
<?php	
	print("opener.location=\"http://technodome.org/material.php?filling=gotobutton&gotocheck=$gotocheck&add_edit_duplicate=false&seenbefore=1\"; ");
 ?>
	window.close();
...
<body onload= "set_attribute()"> <form name = "test" action = "view_material.php" method = "post">
...
$xz=($pagenum-1)*22;
$query_string = "select * from materialone order by matone_name LIMIT 22 OFFSET $xz ";
...
///test for indication from account screen
 ...
 echo ("<TR align = left valign = top BGCOLOR=\"#$color\"><TD>
	...
	<input type=\"submit\" value=\"$button\" name=\"b$button\" style=\" width: 40; height: 15\" onclick = \"select_record(document.test.b$button.name)\" >
	</TD>");
