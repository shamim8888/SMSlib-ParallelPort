...
<?php	
	print("opener.location=\"http://riverine.org/account_add_insert_experiment.php?filling=gotobutton&gotocheck=$gotocheck\"; ");
 ?>
	window.close();
...
<body> <form name = "view_record" action = "table_button.php" method = "post">
...
$query_string = "select * from accounts order by account_id";
...

{
  ...
  for ($column_num =1; $column_num<$column_count; $column_num++)
    { 
      $field_name= array('ACCOUNT TYPE','ACCOUNT NAME',' OFFICE  ADDRESS        ','  HOME ADDRESS  ','PHONE(OFF)','PHONE(RES)','MOBILE');
