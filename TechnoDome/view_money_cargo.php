...
function new_check()
        {
	        ...
	        document.view_record.view_check.value = "new";
...
<div align="center"><input type="submit" value="New" name="newbutton" style=" width: 84; height: 25" onclick= "new_check()"></div>
...
<?php
    ////////////////query for voucherno starts///////////////////////////
      ...
      ////////////////query for voucherno Ends///////////////////////////
        ...
        $query_string = "select * from cargoschedule_part_advance_view where (ship_id ='$shipcargo' and receive_balance!=0) order by schedule_id";
        ...
        if ($view_check=="new")
                {
                        $query_string2 = "select schedule_id,departure_date,ship_id,ship_name,account_id,account_name,from_id,from_loc,to_id,to_loc,mat_one,matone_name,quantity_one,mat_two,mattwo_name,quantity_two,pay_voucher_date,pay_location,fair_rate,advance_tk,part_tk,total_tk,balance_tk,trip_id,unload_date,money_fair_rate,receive_advance,receive_part,receive_total,receive_balance,mreceipt_date,fair_rate_two,money_fair_rate_two from cargoschedule_part_advance_view where (ship_id ='$shipcargo'  and (receive_balance !=0 or receive_balance=NULL)) order by schedule_id";                    ///previous condition was balance_tk !=0........ I changed it to receive_balance!=0
                }

        else
                {
                        $query_string2 = "select schedule_id,departure_date,ship_id,ship_name,account_id,account_name,from_id,from_loc,to_id,to_loc,mat_one,matone_name,quantity_one,mat_two,mattwo_name,quantity_two,pay_voucher_date,pay_location,fair_rate,advance_tk,part_tk,total_tk,balance_tk,trip_id,unload_date,money_fair_rate,receive_advance,receive_part,receive_total,receive_balance,mreceipt_date,fair_rate_two,money_fair_rate_two from cargoschedule_part_advance_view where (ship_id ='$shipcargo' and received_from='$receivedfrom' and (receive_balance !=0 or receive_balance=NULL)) order by schedule_id";                    ///previous condition was balance_tk !=0........ I changed it to receive_balance!=0
                }

        $result = pg_exec($query_string2);

        $testnumrows = pg_numrows($result);
        print($testnumrows);
        ...
        if (TRUE)
                {
                        ...
                        for ($column_num =1; $column_num<=$column_count; $column_num++)
                                {
                                        $field_name= array('','Scdl&nbsp;id','Dpt.&nbsp;Date','Ship&nbsp;ID','&nbsp;&nbsp;Ship&nbsp;name&nbsp;&nbsp;','Account&nbsp;ID','&nbsp;&nbsp;&nbsp;&nbsp;Owner&nbsp;name&nbsp;&nbsp;&nbsp;&nbsp;','From&nbsp;ID','&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;From&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;','To&nbsp;ID','&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;To&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;','Matone&nbsp;ID','&nbsp;&nbsp;&nbsp;Matone&nbsp;Name&nbsp;&nbsp;&nbsp;','Quantity','Mattwo&nbsp;ID','&nbsp;&nbsp;Mattwo&nbsp;Name&nbsp;&nbsp;','Quantity','Voucher&nbsp;Date','Pay&nbsp;Location','Fair&nbsp;rate','Advance&nbsp;tk','Part&nbsp;tk','Total&nbsp;tk','Balance&nbsp;tk','Trip_id','UnloadD&nbsp;ate','Fair&nbsp;rate(receive)','Receive&nbsp;Advance','Receive&nbsp;Part','Receive&nbsp;Total','Receive&nbsp;Balance','Mreceipt&nbsp;Date');
        ...
        if($numrows!=0 and (floor($row[29])!=0 or floor($row[29])==""))
                {
...
<INPUT TYPE="hidden" name="receivetkratetwo" value="<?php echo $receivetkratetwo ?>">

<INPUT TYPE="hidden" name="receivedfrom" value="<?php echo $receivedfrom ?>">
