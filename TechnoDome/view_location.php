...
if ($submitted == 1)
                        ...
                        if ($filling=="viewcheck")
                                {
                                        ...
                                        $subresult = pg_exec("select * from locationfrom order by from_loc");
                                       ...
                                       // echo "alert(now we have to find the real gotocheck...we already got viewcheck)";
                                        $subresult = pg_exec("select * from locationfrom order by from_id");
                                        $subnumrows = pg_numrows($subresult);
        ...
        else
                {
                        ...
                        $query_string = "select * from locationfrom order by from_loc";
...
<?php	
	print("opener.location=\"http://technodome.org/location.php?filling=gotobutton&gotocheck=$gotocheck&add_edit_duplicate=false&seenbefore=1\"; ");
 ?>
	window.close();
...
<body bgcolor="#A6B4A3" onload= "set_attribute()"> <form name = "test" action = "view_location.php" method = "post">
...
$query_string = "select * from locationfrom order by from_loc LIMIT 25 OFFSET $xz ";
