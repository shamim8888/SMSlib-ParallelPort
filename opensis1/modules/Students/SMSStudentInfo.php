	...
	if(count($RET))
	{
		...
		foreach($RET as $student)
		{
			$_SESSION['student_id'] = $student['STUDENT_ID'];
                                                        ...
                                                        $StrToInsSMSText = "Personal Information";
			...
			echo "<td>".$sql['SOCIAL_SEC_NO'] ."</td></tr>";
                        $StrToInsSMSText .= $sql['SOCIAL_SEC_NO'];
					   ...
					   if($contact['HOME_PHONE']!='')
						 ...
						 if($contact['MOBILE_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>Mobile Phone :</td><td>".$contact['MOBILE_PHONE']."</td></tr>";
                                                     $StrToInsSMSRecipient = $contact['MOBILE_PHONE'];
						 }
						 if($contact['EMAIL']!='')
						{
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		echo '</A><INPUT type=checkbox value="Print" name=controller checked onclick="checkAll(this.form,this.form.controller.checked,\'Print\');"><strong>Print </strong><A>';
                echo '</A><INPUT type=checkbox value="Email" name=controller checked onclick="checkAll(this.form,this.form.controller.checked,\'Print\');"><strong>E-mail </strong><A>';
                echo '</A><INPUT type=checkbox value="SMS" name=controller checked onclick="checkAll(this.form,this.form.controller.checked,\'Print\');"><strong>SMS</strong><A>';
                echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\'Print/E-Mail/SMS Info for Selected Students\'></CENTER>';
		echo "</FORM>";
	}
}
...
// My Writing Shamim Ahmed Chowdhury. Delete it after testing
        echo $StrToInsSMSText;
        echo "\n";
        echo $StrToInsSMSRecipient;
        $subject = "Student Info";
        $from = "opensis@localhost";
        $header = "From:" . $from;
        DBQuery("INSERT INTO smsserver_out (recipient,text,status_report) VALUES ('$StrToInsSMSRecipient','$StrToInsSMSText','1') ");  //pinki
        mail($to, $subject, $StrToInsSMSText, $headers);
        // end of my writing
?>
