...
public class Xml extends Interface<File>
{
	...
	public Xml(String myInterfaceId, Properties myProps, SMSServer myServer, InterfaceTypes myType)
	{
		...
		setDescription("Default Interface for XML input/output files");
		/* Read arguments */
	 ...
	 * @param m
	 *            The message to add to the document
	 */
	...
	public Collection<OutboundMessage> getMessagesToSend() throws Exception
	{
		...
		for (int i = 0; i < outFiles.length; i++)
		{
			...
			{
				Logger.getInstance().logWarn("Skipping outgoing file " + outFiles[i].getAbsolutePath() + ": File is not valid: " + e.getLocalizedMessage(), null, null);
				File brokenFile = new File(this.outBrokenDirectory, outFiles[i].getName());
				if (!outFiles[i].renameTo(brokenFile)) Logger.getInstance().logError("Can't move " + outFiles[i] + " to " + brokenFile, null, null);
			}
		}
		return messageList;
	}
	...
	public void markMessage(org.smslib.OutboundMessage msg) throws Exception
	{
		...
		if (f.renameTo(newF))
		{
			Logger.getInstance().logInfo(f + " marked.", null, null);
		}
		else
		{
			Logger.getInstance().logWarn("Can't move " + f + " to " + newF, null, null);
		}
	...
	public void messagesReceived(Collection<InboundMessage> msgList) throws Exception
	{
		...
		for (InboundMessage msg : msgList)
		{
			...
			if ((msg.getType() == MessageTypes.INBOUND) || (msg.getType() == MessageTypes.STATUSREPORT))
			{
				...
				Logger.getInstance().logInfo("Writing inbound files to " + outputFile.getName(), null, null);
	...
	/**
	 * Writes the given document to the given filename. <br />
	 * The DTD smsserver_in.dtd is added, too.
	 * 
	 * @param doc
