...
public class SMSServer
{
	...
	private void loadConfiguration() throws Exception
	{
		...
		try
		{
			...
			if (getProperties().getProperty("smsserver.router", "").length() > 0)
			{
				...
				{
					Logger.getInstance().logError("SMSServer: error setting custom router!", null, null);
				}
			}
	...
	public boolean checkPriorityTimeFrame(int priority)
	{
        ...
        //Added By Shamim Ahmed Chowdhury To Get Host Name 
        public String getHostAddress()
	{
		String HostAddress ;
                HostAddress = getProperties().getProperty("settings.host.address", "");
