...
public class ModemGateway extends AGateway
{
	...
	public enum ModemTypes
	{
		...
		SERIAL,
                /**
		 * Parallel connected modem. These modems are connected via a parallel
		 * port, either physical or emulated (i.e. USB, IrDA, etc).
		 */
		PARALLEL,
		/**
	...
	private void init(ModemTypes myType, String myModemDevice, int myModemParms, String myManufacturer, String myModel, AModemDriver myDriver)
	{
		...
		else
		{
			if (myType == ModemTypes.SERIAL) setDriver(new SerialModemDriver(this, getModemDevice() + ":" + getModemParms()));
                        else if (myType == ModemTypes.PARALLEL) setDriver(new ParallelModemDriver(this, getModemDevice() + ":" + getModemParms()));
			...
			if (getATHandler().switchStorageLocation(getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2)))
			{
				...
				while (true)
				{
					...
					if (Character.isDigit(tokens.nextToken().trim().charAt(0)))
					{
						...
						msg = new StatusReportMessage(refNo, memIndex, getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2), getATHandler().getSerialNo(), cal1.getTime(), cal2.getTime());
						msg.setGatewayId(getGatewayId());
					...
					else
					{
						...
						msg = new InboundMessage(cal1.getTime(), originator, msgText, memIndex, getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2), getATHandler().getSerialNo() );
						msg.setGatewayId(getGatewayId());
						Logger.getInstance().logDebug("IN-DTLS: MI:" + msg.getMemIndex(), null, getGatewayId());
						msgList.add(msg);
						incInboundMessageCount();
			...
			if (getATHandler().switchStorageLocation(getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2)))
			{
				...
				while (true)
				{
					...
					try
					{
						...
						if (pdu instanceof SmsDeliveryPdu)
						{
							...
							String memLocation = getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2);
                                                        String GatewayAddress = getATHandler().getSerialNo();
                                                        //String HostAddress = getServer().getHostAddress();
							if (pdu.isBinary())
							{
								msg = new InboundBinaryMessage((SmsDeliveryPdu) pdu, memIndex, memLocation, GatewayAddress);
								if (Service.getInstance().getKeyManager().getKey(msg.getOriginator()) != null) msg = new InboundEncryptedMessage((SmsDeliveryPdu) pdu, memIndex, memLocation, GatewayAddress);
							}
							else
							{
								msg = new InboundMessage((SmsDeliveryPdu) pdu, memIndex, memLocation, GatewayAddress);
							}
							msg.setGatewayId(getGatewayId());
						...
						else if (pdu instanceof SmsStatusReportPdu)
						{
							...
							msg = new StatusReportMessage((SmsStatusReportPdu) pdu, memIndex, getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2), getATHandler().getSerialNo());
							msg.setGatewayId(getGatewayId());
							msgList.add(msg);
							incInboundMessageCount();
						}
					...
					catch (Exception e)
					{
						...
						UnknownMessage msg;
						msg = new UnknownMessage(pduString, memIndex, getATHandler().getStorageLocations().substring((ml * 2), (ml * 2) + 2), getATHandler().getSerialNo());
						msg.setGatewayId(getGatewayId());
						msgList.add(msg);
						incInboundMessageCount();
