...
public class DesktopApplication1View extends FrameView {
    ...
    public void newDetailRecord() {
        ...
        desktopapplication1.Customer c = list.get(masterTable.convertRowIndexToModel(index));
        Collection<desktopapplication1.PurchaseOrder> ps = c.getPurchaseOrderList();
        if (ps == null) {
            ps = new LinkedList<desktopapplication1.PurchaseOrder>();
            c.setPurchaseOrderList((List)ps);
        }
        desktopapplication1.PurchaseOrder p = new desktopapplication1.PurchaseOrder();
    ...
    public void deleteDetailRecord() {
        ...
        List<desktopapplication1.PurchaseOrder> toRemove = new ArrayList<desktopapplication1.PurchaseOrder>(selected.length);
        for (int idx=0; idx<selected.length; idx++) {
            selected[idx] = detailTable.convertRowIndexToModel(selected[idx]);
            int count = 0;
            Iterator<desktopapplication1.PurchaseOrder> iter = ps.iterator();
            while (count++ < selected[idx]) iter.next();
            desktopapplication1.PurchaseOrder p = iter.next();
            toRemove.add(p);
